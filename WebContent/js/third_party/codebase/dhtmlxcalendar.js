...
if (typeof (window.dhx4.dateLang) == "undefined") {
	...
	window.dhx4.date2str = function(f, d, a) {
		...
		if (f instanceof Date) {
			...
			var c = String(d || window.dhx4.dateFormat)
					.replace(/%[a-zA-Z]/g, b)
		}
		return (c || String(f))
	};
	...
	window.dhx4.ajax = {
		...
		xmltop : function(a, d, c) {
			if (typeof d.status == "undefined" || d.status < 400) {
				xml = (!d.responseXML) ? dhx4.ajax.parse(d.responseText || d)
						: (d.responseXML || d);
		...
		query : function(a) {
			dhx4.ajax._call((a.method || "GET"), a.url, a.data || "",
		...
		_call : function(a, b, c, e, g, j, d) {
			var i = (window.XMLHttpRequest && !dhx4.isIE ? new XMLHttpRequest()
					: new ActiveXObject("Microsoft.XMLHTTP"));
			var f = (navigator.userAgent.match(/AppleWebKit/) != null
		...
		g._loadData = function(n, o, p) {
			...
			if (typeof (n) == "string") {
				...
				if (s.test(l.replace(/^<\?xml[^\?]*\?>\s*/, ""))) {
					m = dhx4.ajax.parse(n);
					if (m != null) {
						m = this[this._dhxdataload.xmlToJson]
								.apply(this, [ m ])
					}
				}
				if (m == null
						&& (l.match(/^[\s\S]*{[.\s\S]*}[\s\S]*$/) != null || l
				...
				if (m == null) {
					...
					if (typeof (this._dhxdataload.onBeforeXLS) == "function") {
						var l = this._dhxdataload.onBeforeXLS
								.apply(this, [ n ]);
					...
					var i = function(a) {
						...
						if ((a.xmlDoc.getResponseHeader("Content-Type") || "")
							...
							k = r[r._dhxdataload.xmlToJson].apply(r,
				...
				if (typeof (n.documentElement) == "object"
						|| (typeof (n.tagName) != "undefined"
								&& typeof (n.getElementsByTagName) != "undefined" && n
...
if (typeof (dhtmlxEvent.initTouch) == "undefined") {
	dhtmlxEvent.initTouch = function() {
		...
		dhtmlxEvent(document.body, "touchmove", function(f) {
			if (d) {
				if (Math.abs(f.touches[0].clientX - b) > 50
...
function dhtmlXCalendarObject(f, k) {
	...
	if (d && typeof (d) == "object" && d.tagName
	...
	for (var b = 0; b < f.length; b++) {
		...
		if (f[b] != null && f[b].tagName
				&& String(f[b].tagName).toLowerCase() == "input") {
			this.i[window.dhx4.newId()] = {
				input : f[b]
			}
		} else {
			if (!(f[b] instanceof Array) && f[b] instanceof Object
	...
	j.onclick = function(n) {
		...
		if (l.className
			...
			e._drawMonth(new Date(e._activeMonth.getFullYear(), e._activeMonth
					.getMonth()
					+ m, 1, 0, 0, 0, 0));
		...
		if (l.className && l.className == "dhtmlxcalendar_month_label_month") {
			n.cancelBubble = true;
			e._showSelector("month", Math.round(l.offsetLeft + l.offsetWidth
					/ 2), l.offsetTop + l.offsetHeight + 2, "selector_month",
					true);
			return
		}
		if (l.className && l.className == "dhtmlxcalendar_month_label_year") {
			n.cancelBubble = true;
			e._showSelector("year", Math
					.round(l.offsetLeft + l.offsetWidth / 2), l.offsetTop
					+ l.offsetHeight + 2, "selector_year", true);
			return
		}
		e._hideSelector()
	};
	...
	this._drawDaysOfWeek = function() {
		...
		for (var o = 0; o < 8; o++) {
			...
			if (o == 0) {
				...
				a.innerHTML = "<div class='dhtmlxcalendar_label'>"
						+ (this.langData[this.lang].weekname || "w") + "</div>"
			} else {
	...
	this.contDates.onclick = function(p) {
		...
		if (l._date != null && !l._css_dis) {
			...
			if (e.checkEvent("onBeforeChange")) {
				if (!e.callEvent("onBeforeChange", [ new Date(l._date
						.getFullYear(), l._date.getMonth(), l._date.getDate(),
						n, m) ])) {
					return
				}
			}
			...
			var a = (e._activeDate.getFullYear() + "_"
					+ e._activeDate.getMonth() != o.getFullYear() + "_"
					+ o.getMonth());
			e._nullDate = false;
			e._activeDate = new Date(o.getFullYear(), o.getMonth(),
					o.getDate(), n, m);
			e._activeDateCell = l;
	...
	j.innerHTML = "<div class='dhtmlxcalendar_time_img'></div><span class='dhtmlxcalendar_label_hours'></span><span class='dhtmlxcalendar_label_colon'>:</span><span class='dhtmlxcalendar_label_minutes'></span><span class='dhtmlxcalendar_label_clear'>"
			+ this.langData[this.lang].clear
			+ "</span><span class='dhtmlxcalendar_label_today'>"
			+ this.langData[this.lang].today + "</span>";
	g.appendChild(j);
	j.onclick = function(m) {
		...
		if (a.tagName != null && a.tagName.toLowerCase() == "span"
				&& a._par == true && a.parentNode != null) {
			a = a.parentNode
		}
		if (a.className && a.className == "dhtmlxcalendar_label_hours") {
			m.cancelBubble = true;
			var l = e.contMonth.offsetHeight + e.contDays.offsetHeight
					+ e.contDates.offsetHeight + a.offsetTop;
			e._showSelector("hours", Math.round(a.offsetLeft + a.offsetWidth
					/ 2), l - 2, "selector_hours", true);
			return
		}
		if (a.className && a.className == "dhtmlxcalendar_label_minutes") {
			...
			if (e._minutesInterval == 1) {
				...
				a.innerHTML = "<span class='dhtmlxcalendar_selected_date'>"
						+ n.charAt(0) + "</span>" + n.charAt(1);
				a.firstChild._par = true;
				e._selectorMode = 1
			}
			var l = e.contMonth.offsetHeight + e.contDays.offsetHeight
					+ e.contDates.offsetHeight + a.offsetTop;
			e._showSelector("minutes", Math.round(a.offsetLeft + a.offsetWidth
					/ 2), l - 2, "selector_minutes", true);
			return
		}
		e._hideSelector();
	...
	this._drawMonth = function(u) {
		...
		if (isNaN(u.getFullYear())) {
			u = new Date(this._activeMonth.getFullYear(), this._activeMonth
					.getMonth(), 1, 0, 0, 0, 0)
		}
		this._activeMonth = new Date(u.getFullYear(), u.getMonth(), 1, 0, 0, 0,
				0);
		this._activeDateCell = null;
		...
		var B = u.getMonth();
		var C = new Date(this._activeDate.getFullYear(), this._activeDate
		...
		for (var l = 0; l < 6; l++) {
			...
			for (var A = 0; A <= 7; A++) {
				if (A == 0) {
					var z = this.getWeekNumber(new Date(s.getFullYear(), s
							.getMonth(), s.getDate() + p, 0, 0, 0, 0));
					if (z >= 52 && this.conf.ws_first == true
							&& this._activeMonth.getMonth() == 0) {
						var o = this.getWeekNumber(new Date(s.getFullYear(), s
								.getMonth(), s.getDate() + p + 7, 0, 0, 0, 0));
						if (o < z && o > 1) {
							z = 1
						}
					}
					this.contDates.childNodes[l].childNodes[A].innerHTML = "<div class='dhtmlxcalendar_label'>"
							+ z + "</div>"
				} else {
					var a = new Date(s.getFullYear(), s.getMonth(), s.getDate()
							+ p, 0, 0, 0, 0);
					if (a.getHours() != 0) {
						var r = (a.getHours() > 12 ? 24 - a.getHours() : a
								.getHours());
					...
					if (this._tipData[n] != null) {
						if (this._tipData[n].usePopup
								&& typeof (window.dhtmlXPopup) == "function") {
							this.contDates.childNodes[l].childNodes[A]
					...
					this.contDates.childNodes[l].childNodes[A].innerHTML = "<div class='"
							+ v + "'>" + a.getDate() + "</div>";
					this.contDates.childNodes[l].childNodes[A]._date = new Date(
							n);
					this.contDates.childNodes[l].childNodes[A]._q = l;
					this.contDates.childNodes[l].childNodes[A]._w = A;
					this.contDates.childNodes[l].childNodes[A]._css_month = (a
							.getMonth() == B);
					...
					this.contDates.childNodes[l].childNodes[A]._css_dis = this
							._isOutOfRange(n);
	...
	this._initSelector = function(s, n) {
		...
		if (s == "month") {
			...
			this.msCont.onclick = function(v) {
				...
				if (q._month != null) {
					...
					e._drawMonth(new Date(e._activeMonth.getFullYear(),
							q._month, 1, 0, 0, 0, 0));
		...
		if (s == "year") {
			...
			this.ysCont.onclick = function(v) {
				...
				if (q._year != null) {
					e._hideSelector();
					e._drawMonth(new Date(q._year, e._activeMonth.getMonth(),
							1, 0, 0, 0, 0));
					e._doOnSelectorChange()
				}
			};
		...
		if (s == "hours") {
			...
			this.hsCont.onclick = function(v) {
				...
				if (q._hours != null) {
					...
					e._doOnSelectorChange();
					e.callEvent("onTimeChange", [ new Date(e._activeDate
							.getTime()) ])
				}
			};
			if (this.conf.touch == true) {
				...
				if (v._minutes != null) {
					if (e._minutesInterval == 1) {
						...
						if (e._selectorMode == 1) {
							...
							e._selectorMode = 2;
							e._showSelector("minutes", e._sel._x, e._sel._y,
	...
	this._showSelector = function(o, l, p, n, a) {
		...
		if (o != this._sel._t && this._sel._t == "minutes"
		...
		this._sel.className = "dhtmlxcalendar_selector_obj dhtmlxcalendar_" + n
				+ (o == "hours" && this.conf.today == true ? "2" : "");
		this._sel.childNodes[0].firstChild.firstChild.childNodes[0].style.display = this._sel.childNodes[0].firstChild.firstChild.childNodes[2].style.display = (o == "year" ? ""
				: "none");
	...
	this._scrollYears = function(a) {
		var m = (a < 0 ? this.ysCont.firstChild.firstChild._year
	...
	this.setDateFormat = function(o) {
		...
		this._dateFormat = o;
		var m = String(this._dateFormat).replace(
				/%[a-zA-Z]+/g,
				function(a) {
					var q = a.replace(/%/, "");
					switch (q) {
					...
					case "M":
						return "("
								+ e.langData[e.lang].monthesSNames.join("|")
										.toLowerCase() + "){1,}";
					case "F":
						return "("
								+ e.langData[e.lang].monthesFNames.join("|")
										.toLowerCase() + "){1,}";
					case "D":
						return "[a-z]{2}";
					case "a":
	...
	this.getWeekNumber = function(p) {
		...
		var x = 604800000;
		var l = new Date(p.getFullYear(), p.getMonth(), p.getDate() + 1, 0, 0,
				0, 0);
		var w = Math.ceil((l.getTime() - t.getTime()) / x);
		if (w == 0) {
			var q = new Date(p.getFullYear(), p.getMonth(), p.getDate(), 0, 0,
					0, 0);
	...
	this.show = function(n) {
		...
		if (typeof (n) == "string") {
			...
			if (m != null && typeof (m._dhtmlxcalendar_uid) != "undefined"
		...
		if (typeof (n) == "object"
				&& typeof (n._dhtmlxcalendar_uid) != "undefined"
				&& this.i[n._dhtmlxcalendar_uid] == n) {
	...
	this._show = function(q, l) {
		...
		if (!q) {
			...
			if (this.pos == "right") {
				...
				this.base.style.top = Math.min(p.top, r.bottom
				...
				if (this.pos == "bottom") {
					...
					if (a + this.base.offsetWidth > r.right) {
						a = Math.max(0, p.left + m.offsetWidth
	...
	this.setInsensitiveDays = function(m) {
		...
		for (var l = 0; l < a.length; l++) {
			this._rangeSet[new Date(a[l].getFullYear(), a[l].getMonth(), a[l]
		...
		if (l == null) {
			...
			if (l != null) {
				...
				for (var m = 0; m < a.length; m++) {
					this._holidays[new Date(a[m].getFullYear(),
							a[m].getMonth(), a[m].getDate(), 0, 0, 0, 0)
							.getTime()] = true
				}
			}
		}
	...
	this.setSensitiveRange = function(n, m, a) {
		...
		if (n != null && m != null) {
			...
			this._rangeFrom = new Date(n.getFullYear(), n.getMonth(), n
					.getDate(), 0, 0, 0, 0).getTime();
			this._rangeTo = new Date(m.getFullYear(), m.getMonth(),
					m.getDate(), 0, 0, 0, 0).getTime();
	...
	this.setInsensitiveRange = function(l, a) {
		if (l != null && a != null) {
			...
			this._rangeFrom = new Date(l.getFullYear(), l.getMonth(), l
					.getDate(), 0, 0, 0, 0).getTime();
	...
	this.setTooltip = function(m, p, r, a) {
		...
		for (var o = 0; o < n.length; o++) {
			var l = new Date(n[o].getFullYear(), n[o].getMonth(), n[o]
					.getDate(), 0, 0, 0, 0).getTime();
			this._tipData[l] = {
				text : p,
				showIcon : r,
	...
	this.clearTooltip = function(l) {
		...
		for (var n = 0; n < m.length; n++) {
			var a = new Date(m[n].getFullYear(), m[n].getMonth(), m[n]
					.getDate(), 0, 0, 0, 0).getTime();
	...
	this._initTooltipPopup = function() {
		...
		this.attachEvent("onMouseOver", function(l) {
			var a = new Date(l.getFullYear(), l.getMonth(), l.getDate(), 0, 0,
					0, 0).getTime();
	...
	this._showTooltipPopup = function(n, a, o, l, m) {
		...
		e._tipPopup.show(e._tipCellDim.x, e._tipCellDim.y, e._tipCellDim.w,
				e._tipCellDim.h);
	...
	this.getCellDimension = function(m) {
		...
		var n = new Date(m.getFullYear(), m.getMonth(), m.getDate(), 0, 0, 0, 0)
				.getTime();
		var l = null;
	...
	this._doOnClick = function(l) {
		l = l || event;
		var a = (l.target || l.srcElement);
		if (a._dhtmlxcalendar_uid && a._dhtmlxcalendar_uid != e._activeInp
				&& e._isVisible() && e._activeInp) {
			e._hide();
			return
		}
	...
	this.attachObj = function(m) {
		...
		if (typeof (m) == "string") {
			...
			if (typeof (m.tagName) != "undefined") {
				...
				if (typeof (m) == "object"
						&& (m.input != null || m.button != null)) {
					this.i[l] = {};
					if (m.input != null) {
						this.i[l].input = (typeof (m.input) == "string" ? document
								.getElementById(m.input)
								: m.input)
					}
					if (m.button != null) {
						this.i[l].button = (typeof (m.button) == "string" ? document
		...
		if (this.i[n] != null) {
			...
			if (typeof (n) == "string") {
				...
				if (typeof (n.tagName) != "undefined") {
					...
					if (typeof (n) == "object"
							&& (n.input != null || n.button != null)) {
						if (m == null && n.input != null) {
							m = (typeof (n.input) == "string" ? document
									.getElementById(n.input) : n.input)._dhtmlxcalendar_uid
						}
						if (m == null && n.button != null) {
							m = (typeof (n.button) == "string" ? document
									.getElementById(n.button) : n.button)._dhtmlxcalendar_uid
						}
					}
				}
			}
	...
	this._attachEventsToObject = function(l) {
		if (this.i[l].button != null) {
			...
			if (typeof (window.addEventListener) == "function") {
				this.i[l].button.addEventListener("click", e._doOnBtnClick,
						false);
				if (this.conf.touch == true) {
					this.i[l].button.addEventListener("touchstart",
							e._doOnBtnClick, false)
				}
			} else {
			...
			if (this.i[l].input != null) {
				...
				if (typeof (window.addEventListener) == "function") {
					this.i[l].input.addEventListener("click", e._doOnInpClick,
							false);
					this.i[l].input.addEventListener("keyup", e._doOnInpKeyUp,
							false);
					if (this.conf.touch == true) {
						this.i[l].input.addEventListener("touchstart",
								e._doOnInpClick, false)
					}
				} else {
	...
	this._detachEventsFromObject = function(l) {
		if (this.i[l].button != null) {
			if (typeof (window.addEventListener) == "function") {
				this.i[l].button.removeEventListener("click", e._doOnBtnClick,
						false);
				if (this.conf.touch == true) {
					this.i[l].button.removeEventListener("touchstart",
							e._doOnBtnClick, false)
				}
			} else {
			...
			if (this.i[l].input != null) {
				if (typeof (window.addEventListener) == "function") {
					this.i[l].input.removeEventListener("click",
							e._doOnInpClick, false);
					this.i[l].input.removeEventListener("keyup",
							e._doOnInpKeyUp, false);
					if (this.conf.touch == true) {
						this.i[l].input.removeEventListener("touchstart",
								e._doOnInpClick, false)
					}
				} else {
	...
	this.unload = function() {
		...
		if (typeof (window.addEventListener) == "function") {
			...
			if (this.conf.touch == true) {
				document.body.removeEventListener("touchstart", e._doOnClick,
						false)
			}
		} else {
...
dhtmlXCalendarObject.prototype._buildMonthHdr = function(e) {
	...
	var a = function(f, g) {
		return "<span class='dhtmlxcalendar_month_label_" + f + "'>" + g
				+ "</span>"
	};
	var c = String(this.langData[this.lang].hdrformat || ("%F %Y")).replace(
			...
			function(f) {
				...
				case "%M":
					return a("month", b.langData[b.lang].monthesSNames[e
							.getMonth()]);
				case "%F":
					return a("month", b.langData[b.lang].monthesFNames[e
							.getMonth()]);
				case "%y":
